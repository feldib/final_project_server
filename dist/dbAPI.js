...
const getRegisteredUsers = async () => {
    const connection = await makeConnection();
    const [users] = await connection.execute(`SELECT id, last_name, first_name, email, address, phone_number FROM users
...
const getSpecificTags = async (artwork_id) => {
    const connection = await makeConnection();
    const [tags] = await connection.query(`SELECT tags.id, tags.tname 
    FROM tags 
    LEFT JOIN artwork_tags ON artwork_tags.tag_id = tags.id
    WHERE artwork_id=? 
    AND artwork_tags.removed = false 
    AND tags.removed = false;`, [artwork_id]);
    connection.end();
    return tags;
};
...
const getFeatured = async (n) => {
    ...
    ${n ? ` LIMIT ${n}` : ""}
  `);
    const [results] = await connection.query(`SELECT id, title, price FROM artworks WHERE id IN (${artwork_ids
        .map((obj) => "?")
...
const getThumbnail = async (id) => {
    const connection = await makeConnection();
    const [thumbnail] = await connection.query(`SELECT picture_path FROM artwork_pictures WHERE artwork_id = ? AND is_thumbnail = true`, [id]);
    connection.end();
    return thumbnail[0].picture_path;
};
const getOtherPictures = async (artwork_id) => {
    const connection = await makeConnection();
    const [results] = await connection.query(`SELECT picture_path FROM artwork_pictures WHERE artwork_id = ? AND is_thumbnail = false`, [artwork_id]);
...
const checkIfRegistered = async (email) => {
    const connection = await makeConnection();
    const [results, fields] = await connection.query(`SELECT id, is_admin FROM users WHERE email = ?;`, [email]);
    connection.end();
...
const checkEmail = async (email) => {
    const connection = await makeConnection();
    const [results, fields] = await connection.query(`SELECT id FROM users WHERE email = ?;`, [email]);
    connection.end();
    if (results.length !== 0) {
        return {
            registered: true,
...
const getReviewsOfArtwork = async (artwork_id) => {
    const connection = await makeConnection();
    const [reviews] = await connection.query(`SELECT CONCAT(users.last_name, " ", users.first_name) 'name', reviews.id, 
      ...
      WHERE reviews.artwork_id = ? AND reviews.approved = true AND reviews.removed = false`, [artwork_id]);
    connection.end();
    return reviews;
};
const getUnapprovedReviews = async () => {
    const connection = await makeConnection();
    const [reviews] = await connection.execute(`SELECT CONCAT(users.last_name, " ", users.first_name) 'name', reviews.id, 
      reviews.user_id, reviews.time_review_posted, reviews.title, reviews.review_text,
...
const getReviewsOfUser = async (user_id) => {
    const connection = await makeConnection();
    const [reviews] = await connection.query(`SELECT reviews.id, reviews.time_review_posted, reviews.title, 
      artworks.id as artwork_id, artworks.title as artwork_title,
      artworks.artist_name, reviews.approved, reviews.review_text
      FROM reviews LEFT JOIN users ON reviews.user_id = users.id
      LEFT JOIN artworks ON reviews.artwork_id = artworks.id
      WHERE reviews.user_id = ? AND reviews.removed = false`, [user_id]);
    connection.end();
    return reviews;
};
const getDataOfArtwork = async (id) => {
    const connection = await makeConnection();
    const [artworks] = await connection.query(`SELECT artwork_pictures.picture_path 'thumbnail', categories.cname, 
    ...
    AND artwork_pictures.is_thumbnail = true`, [id]);
...
const updateUserData = async (user_id, field_name, value) => {
    const connection = await makeConnection();
    if (["first_name", "last_name", "email", "address", "phone_number"].includes(field_name)) {
        await connection.query(`
...
const getUnansweredMessages = async () => {
    const connection = await makeConnection();
    const [messages] = await connection.execute(`SELECT id, email, message_title, message_txt, message_time
...
export { getUser, getCategories, searchArtworks, getFeatured, getThumbnail, checkIfRegistered, registerUser, checkEmail, sendLinkToResetPassword, resetPassword, getUserWithId, verifyPaswordToken, verifyUser, getDataOfArtwork, getReviewsOfArtwork, saveMessgeToAdministrator, checkIfArtworkInStock, addToShoppingList, getShoppingListItems, getSpecificCategory, setShoppingCartItemQuantityToZero, increaseShoppingCartItemQuantity, decreaseShoppingCartItemQuantity, addToWishlisted, removeFromWishlisted, getWishlisted, checkIfWishlisted, updateUserData, makeOrder, getOrdersOfUser, leaveReview, getUnapprovedReviews, verifyAdmin, approveReview, removeReview, getReviewsOfUser, getOrders, getUnansweredMessages, sendReplyToMessage, getRegisteredUsers, removeArtwork, checkIfFeatured, addToFeatured, removeArtworkFromFeatured, addNewArtwork, addArtworkTags, };
