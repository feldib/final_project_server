...
export const setShoppingCartItemQuantityToZero = async (user_id, artwork_id) => {
    const connection = await makeConnection();
...
export const updateArtworkTags = async (artwork_id, tags) => {
    ...
    const [tags_of_artwork] = await connection.query(`
      ...
      `, [artwork_id]);
    const tagsToAdd = tags.filter((tag) => !tags_of_artwork.some((tg) => tg.tname === tag));
...
//# sourceMappingURL=change_data.js.map