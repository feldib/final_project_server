...
export const getWishlistedTheMost = async (n) => {
    const connection = await makeConnection();
    const [results] = await connection.execute(`SELECT times_wishlisted, artworks.id, artworks.title, artworks.price, artworks.quantity, artworks.artist_name
    FROM artworks 
    LEFT JOIN
      (
...
export const getDataOfArtwork = async (id) => {
    const connection = await makeConnection();
    const [artworks] = await connection.query(`SELECT categories.cname, 
    ...
    WHERE artworks.id=? 
    AND categories.removed = false`, [id]);
