...
const searchArtworks = async (
  ...
  let sql_query =
    "SELECT id, title, artist_name, price, quantity, category_id, date_added FROM artworks WHERE removed=false";
...
const getFeatured = async (n) => {
  ...
  const [results] = await connection.query(
    `SELECT id, title, price FROM artworks WHERE id IN (${artwork_ids
      .map((obj) => "?")
      .join(", ")})`,
...
const getThumbnail = async (id) => {
  ...
  const [thumbnail] = await connection.query(
    `SELECT picture_path FROM artwork_pictures WHERE artwork_id = ? AND is_thumbnail = true`,
    [id]
  );
  connection.end();
...
const getOtherPictures = async (artwork_id) => {
  const connection = await makeConnection();
  const [results] = await connection.query(
    `SELECT picture_path FROM artwork_pictures WHERE artwork_id = ? AND is_thumbnail = false`,
    [artwork_id]
  );
